...
from advanced_semantic_chunking import AdvancedSemanticChunker, SemanticChunk
...
class ImageExtractionPipeline:
            ...
            for page_num in range(len(doc)):
                ...
                for img_index, img in enumerate(image_list):
                    try:
                        ...
                        img_rect = page.get_image_rects(xref)[0] if page.get_image_rects(xref) else fitz.Rect(0, 0, pix.width, pix.height)
                        ...
                        try:
                            description, confidence = await self.vision_model.describe_image(
                                pil_image, 
                                context=f"technical document page {page_num + 1}"
                            )
                        except Exception as e:
    ...
    def _extract_technical_elements(self, image: Image.Image, description: str) -> List[str]:
        ...
        technical_keywords = [
            ...
            "topology", "configuration", "interface", "protocol", "connection",
            "server", "database", "router", "switch", "cable", "port",
            "tcp", "ip", "ethernet", "wifi", "wireless", "antenna"
        ]
...
class EnhancedTableProcessor:
    ...
    def _is_technical_table(self, df: pd.DataFrame, summary: str) -> bool:
        ...
        technical_indicators = [
            "parameter", "config", "setting", "value", "specification",
            "port", "address", "protocol", "version", "status",
            "tcp", "ip", "mac", "ethernet", "wifi", "database",
            "server", "router", "switch", "interface", "connection"
        ]
        ...
        tech_count = sum(1 for indicator in technical_indicators 
                        if indicator in table_text or indicator in summary_text)
...
class MultimodalSearchEngine:
    ...
    async def multimodal_search(
        ...
        try:
            ...
            combined_results = self._combine_multimodal_results(
                text_results, multimodal_matches, query
            )
            ...
            logger.info(f"Multimodal search completed: {len(final_results)} results, "
    ...
    def _combine_multimodal_results(
        ...
        multimodal_matches: List[MultimodalContent],
        query: str
    ) -> List[SearchResult]:
