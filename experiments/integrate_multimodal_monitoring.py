...
class MultimodalSystemInstrumentation:
    ...
    async def setup_comprehensive_monitoring(self):
        ...
        try:
            ...
            from phase3a_multimodal_simple import Phase3AMultimodalSystem
    ...
    def generate_monitoring_report(self) -> Dict[str, Any]:
        ...
        report = {
            "monitoring_report": {
                ...
                "component_status": {
                    "vision_model": "instrumented"
                    if "vision_model_manager" in self.instrumented_components
                    else "not_instrumented",
                    "embedding_generator": "instrumented"
                    if "embedding_generator" in self.instrumented_components
                    else "not_instrumented",
                    "search_engine": "instrumented"
                    if "search_engine" in self.instrumented_components
                    else "not_instrumented",
                },
            }
        }
...
async def main():
    ...
    logger.info(
        f"  Status: {'Active' if report['monitoring_report']['system_status']['monitoring_active'] else 'Inactive'}"
    )
    logger.info(
        f"  Prometheus: {'Enabled' if report['monitoring_report']['system_status']['prometheus_enabled'] else 'Disabled'}"
    )
    ...
    monitoring.metrics_collector.record_vision_model_performance(
        model_name="test_model", content_type="image", operation="test_operation", latency=1.2, confidence=0.85
    )

    monitoring.metrics_collector.record_embedding_generation(
        content_type="text", model_name="ollama_embed", dimension=768, latency=0.3, cache_hit=True
    )

    monitoring.metrics_collector.record_cross_modal_similarity(
        content_type_1="text", content_type_2="image", similarity_score=0.67, is_cross_modal=True
    )
