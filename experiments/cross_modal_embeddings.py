...
try:
    from phase3a_multimodal_simple import ContentType, MultimodalContent
...
class CrossModalEmbeddingGenerator:
    ...
    async def generate_text_embedding(self, text: str, model: EmbeddingModel = None) -> EmbeddingVector:
        ...
        try:
            ...
            # Cache result
            self.embedding_cache[cache_key] = {"embedding": embedding, "confidence": 0.9, "timestamp": datetime.now()}
        ...
        try:
            ...
            if description:
                text_embedding = await self.generate_text_embedding(
                    f"Image: {description}", EmbeddingModel.OLLAMA_EMBED
                )
                ...
                return EmbeddingVector(
                    ...
                    embedding_dim=len(embedding),
                    confidence=0.4,
                    metadata={"image_size": image.size, "method": "basic_properties"},
                )
    ...
    async def generate_table_embedding(self, table_data: pd.DataFrame, summary: str = "") -> EmbeddingVector:
        ...
        try:
            ...
            text_embedding = await self.generate_text_embedding(combined_text, EmbeddingModel.OLLAMA_EMBED)
            ...
            return EmbeddingVector(
                ...
                embedding_dim=text_embedding.embedding_dim,
                confidence=0.8,
                metadata={"table_shape": table_data.shape, "summary": summary, "text_length": len(combined_text)},
            )
    ...
    async def _generate_ollama_embedding(self, text: str) -> np.ndarray:
        ...
        try:
            ...
            response = requests.post(endpoint, json={"model": model_name, "input": text}, timeout=30)
...
class CrossModalSimilarityCalculator:
    ...
    def calculate_similarity(
        self, embedding1: EmbeddingVector, embedding2: EmbeddingVector, method: str = "cosine"
        ...
        try:
            ...
            if embedding1.embedding_dim != embedding2.embedding_dim:
                logger.warning(
                    f"Embedding dimension mismatch: {embedding1.embedding_dim} vs {embedding2.embedding_dim}"
                )
            ...
            if method == "cosine":
                similarity = cosine_similarity(
                    embedding1.embedding.reshape(1, -1), embedding2.embedding.reshape(1, -1)
                )[0, 0]
    ...
    def batch_similarity(
        self, embeddings: List[EmbeddingVector], query_embedding: EmbeddingVector, top_k: int = 10
...
class CrossModalEmbeddingStore:
    ...
    def search_similar(
        self, query_embedding: EmbeddingVector, content_types: Optional[List[ContentType]] = None, top_k: int = 10
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        for embedding in self.embeddings.values():
            ...
            stats["model_distribution"][model_name] = stats["model_distribution"].get(model_name, 0) + 1
        ...
        for content in contents:
            try:
                ...
                if content.content_type in [ContentType.TEXT]:
                    embedding = await self.embedding_generator.generate_text_embedding(content.text_content)
    ...
    async def cross_modal_search(
        ...
        try:
            ...
            similar_embeddings = self.embedding_store.search_similar(
                query_embedding, content_types, top_k * 2  # Get more candidates for filtering
            )
...
async def main():
    ...
    if PHASE3A_AVAILABLE:
        from phase3a_multimodal_simple import ImageMetadata, MultimodalContent, TableMetadata
    ...
    for query in test_queries:
        ...
        try:
            ...
            for i, result in enumerate(results[:3]):
                logger.info(
                    f"  Result {i+1}: {result.metadata.get('content_type', 'unknown')} "
