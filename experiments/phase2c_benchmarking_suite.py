...
from phase2c_accuracy_system import AccuracyMetrics, Phase2CAccuracySystem, SearchMethod, SearchResult
...
class GroundTruthGenerator:
    ...
    def _calculate_ideal_confidence(self, complexity: str) -> float:
        ...
        complexity_scores = {"simple": 0.9, "moderate": 0.8, "complex": 0.7, "expert": 0.65}
...
class AccuracyEvaluator:
    ...
    def calculate_recall_at_k(self, results: List[SearchResult], ground_truth: List[str], k: int = 5) -> float:
    ...
    def calculate_precision_at_k(self, results: List[SearchResult], ground_truth: List[str], k: int = 5) -> float:
    ...
    def calculate_ndcg(self, results: List[SearchResult], ground_truth: List[str], k: int = 5) -> float:
    ...
    def calculate_mrr(self, results: List[SearchResult], ground_truth: List[str]) -> float:
    ...
    def calculate_ground_truth_match(self, results: List[SearchResult], ground_truth: List[str]) -> float:
...
class PerformanceBenchmarker:
    ...
    async def benchmark_response_time(
        self, accuracy_system: Phase2CAccuracySystem, queries: List[str], method: SearchMethod, iterations: int = 10
    ) -> Dict[str, float]:
        ...
        for iteration in range(iterations):
            for query in queries:
                ...
                try:
                    results, metrics = await accuracy_system.comprehensive_search(query, method=method)
        ...
        async def worker():
            ...
            while time.time() - start_time < duration_seconds:
                ...
                try:
                    ...
                    results.append({"success": True, "response_time": response_time, "timestamp": time.time()})
                    request_count += 1

                except Exception as e:
                    results.append({"success": False, "error": str(e), "timestamp": time.time()})
...
class ComprehensiveBenchmarkSuite:
    ...
    async def run_accuracy_benchmark(self, methods: List[SearchMethod], query_count: int = 20) -> Dict[str, Any]:
            ...
            for benchmark_query in benchmark_queries:
                try:
                    ...
                    results, metrics = await self.accuracy_system.comprehensive_search(
                        benchmark_query.query, method=method
                    )
                    ...
                    ndcg_at_5 = self.accuracy_evaluator.calculate_ndcg(results, benchmark_query.expected_documents, k=5)
                    mrr = self.accuracy_evaluator.calculate_mrr(results, benchmark_query.expected_documents)
                    ...
                    logger.error(
                        f"Benchmark failed for query '{benchmark_query.query}' with method {method.value}: {e}"
                    )
                ...
                for j in range(i + 1, len(method_names)):
                    ...
                    if method_a_results and method_b_results:
                        test = self.statistical_analyzer.compare_methods(
                            method_a_results, method_b_results, MetricType.CONFIDENCE_SCORE
                        )
    ...
    async def run_performance_benchmark(
        self, methods: List[SearchMethod], test_queries: List[str] = None
    ) -> Dict[str, Any]:
        ...
        for method in methods:
            ...
            response_time_results = await self.performance_benchmarker.benchmark_response_time(
                self.accuracy_system, test_queries, method, iterations=5
            )

            # Throughput benchmark
            throughput_results = await self.performance_benchmarker.benchmark_throughput(
                self.accuracy_system, test_queries, method, concurrent_requests=5, duration_seconds=30
            )
    ...
    async def run_comprehensive_benchmark(
        self, methods: List[SearchMethod] = None, query_count: int = 15
    ) -> Dict[str, Any]:
        """Run comprehensive benchmark including accuracy and performance."""

        if methods is None:
            methods = [SearchMethod.VECTOR_ONLY, SearchMethod.ENHANCED_RETRIEVAL, SearchMethod.HYBRID_SEARCH]
    ...
    def _generate_recommendations(
        self, accuracy_results: Dict[str, Any], performance_results: Dict[str, Any]
    ) -> List[str]:
        ...
        if accuracy_data:
            best_accuracy_method = max(accuracy_data.keys(), key=lambda m: accuracy_data[m].get("avg_confidence", 0))
            best_accuracy_score = accuracy_data[best_accuracy_method].get("avg_confidence", 0)

            recommendations.append(
                f"Best accuracy method: {best_accuracy_method} " f"(avg confidence: {best_accuracy_score:.3f})"
            )
        ...
        if performance_data:
            ...
            recommendations.append(
                f"Best performance method: {best_performance_method} " f"(avg response time: {best_response_time:.3f}s)"
            )
        ...
        if significant_tests:
            recommendations.append(f"Found {len(significant_tests)} statistically significant differences")
        ...
        if accuracy_data and performance_data:
            ...
            if method_scores:
                ...
                recommendations.append(f"Recommended method (balanced accuracy/performance): {best_overall}")
...
async def main():
    ...
    # Methods to test
    test_methods = [SearchMethod.ENHANCED_RETRIEVAL, SearchMethod.HYBRID_SEARCH]

    # Run comprehensive benchmark
    results = await suite.run_comprehensive_benchmark(methods=test_methods, query_count=8)  # Reduced for testing
    ...
    for method, metrics in accuracy_results.items():
        logger.info(
            f"{method}: confidence={metrics.get('avg_confidence', 0):.3f}, "
