...
try:
    from phase3a_multimodal_simple import ContentType, ImageMetadata, MultimodalContent, TableMetadata
...
class SimpleCrossModalEmbeddingGenerator:
        ...
        try:
            ...
            if description:
                ...
                return EmbeddingVector(
                    ...
                    confidence=0.4,
                    metadata={"properties": image_properties or {}, "method": "properties_only"},
                )
    ...
    async def _generate_ollama_embedding(self, text: str) -> np.ndarray:
        ...
        try:
            ...
            response = requests.post(endpoint, json={"model": model_name, "prompt": text}, timeout=30)
            ...
            try:
                ...
                if content.content_type in [ContentType.TEXT]:
                    ...
                    embedding = await self.embedding_generator.generate_image_embedding(
                        content.description or content.text_content, image_props
                    )

                elif content.content_type == ContentType.TABLE:
                    embedding = await self.embedding_generator.generate_table_embedding(
                        content.table_data, content.description or content.text_content
                    )
    ...
    async def cross_modal_search(
        ...
        try:
            ...
            for candidate in candidates:
                similarity = self.similarity_calculator.calculate_similarity(query_embedding, candidate)
...
async def main():
    ...
    test_queries = ["network configuration", "router diagram", "IP address table", "TCP protocol setup"]
