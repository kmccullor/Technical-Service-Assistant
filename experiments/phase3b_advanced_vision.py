        ...
        **kwargs,
        ...
        try:
            ...
            result = VisionAnalysisResult(
                ...
                confidence=confidence,
                processing_time=processing_time,
                metadata={"model_name": self.config.model_name, "prompt": prompt, "conversational": True},
                technical_elements=technical_elements,
            )
        ...
        **kwargs,
        ...
        try:
            ...
            if task_type == VisionTaskType.TECHNICAL_ANALYSIS:
                ...
                system_prompt = (
                    "Describe this image in detail, focusing on technical elements and important visual information."
                )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{base_url}/api/generate", json=payload, timeout=self.config.timeout
                ) as response:
                    if response.status == 200:
...
class AdvancedVisionModelManager:
    ...
    def _get_model_capabilities(self, model_type: VisionModelType) -> List[str]:
        ...
        capabilities_map = {
            VisionModelType.BLIP2: ["image_captioning", "object_detection", "visual_understanding"],
            VisionModelType.LLAVA: ["visual_question_answering", "conversational_ai", "chart_reading"],
            VisionModelType.OLLAMA_VISION: ["technical_analysis", "diagram_interpretation", "text_extraction"],
        }
        return capabilities_map.get(model_type, [])

    def get_statistics(self) -> Dict[str, Any]:
        """Get comprehensive statistics for all vision models."""
        stats = {"total_models": len(self.models), "default_model": self.default_model.value, "model_details": {}}
