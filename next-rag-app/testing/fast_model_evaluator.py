...
class FastModelEvaluator:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    f"{instance_url}/api/pull", json=pull_data, timeout=600  # 10 minutes timeout for model pulling
                ) as response:
                    if response.status == 200:
    ...
    async def test_model_performance(
        self, model: str, instance_url: str, questions: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """Test a model's performance on standard questions"""
        questions = questions or self.test_questions[:5]  # Use first 5 for speed
        results = {"response_times": [], "success_count": 0, "total_questions": len(questions), "responses": []}
        ...
        async with aiohttp.ClientSession() as session:
            for i, question in enumerate(questions, 1):
                ...
                try:
                    ...
                    async with session.post(
                        f"{instance_url}/api/generate", json=generate_data, timeout=30  # 30 second timeout
                    ) as response:
                        ...
                        if response.status == 200:
                            ...
                            results["responses"].append(
                                {"question": question, "response": response_text, "response_time": response_time}
                            )
    ...
    def calculate_quality_score(self, responses: List[Dict]) -> float:
        ...
        for resp in responses:
            ...
            if "install" in question and any(
                word in response_text.lower() for word in ["install", "setup", "configure", "download"]
            ):
                score += 0.2
            elif "what is" in question and any(
                word in response_text.lower() for word in ["is", "system", "platform", "software"]
            ):
                score += 0.2
            elif "how" in question and any(
                word in response_text.lower() for word in ["step", "process", "procedure", "follow"]
            ):
                score += 0.2
    ...
    async def evaluate_all_models(self) -> List[ModelTestResult]:
        ...
        for model in self.test_models:
            ...
            if performance["response_times"]:
                ...
                overall_score = (
                    (1.0 / max(avg_response_time, 0.1)) * 0.4
                    + success_rate * 0.3  # Speed (40%)
                    + quality_score * 0.2  # Reliability (30%)
                    + token_efficiency * 0.1  # Quality (20%)  # Efficiency (10%)
                )
    ...
    def generate_recommendations(self, results: List[ModelTestResult]) -> Dict[str, Any]:
        ...
        for result in sorted_results:
            if result.avg_response_time < 10.0 and result.success_rate > 0.8 and result.quality_score > 0.5:
...
async def main():
    ...
    results_file = Path(
        f"/home/kmccullor/Projects/Technical-Service-Assistant/next-rag-app/testing/model_evaluation_{timestamp}.json"
    )
