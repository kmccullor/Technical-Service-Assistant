            ...
            if result_files:
                ...
                try:
                    ...
                    if current_count > last_results_count:
                        ...
                        print(
                            f"   Success Rate: {successful_tests}/{current_count} ({successful_tests/current_count*100:.1f}%)"
                        )
                        print(f"   Average Confidence: {avg_confidence*100:.1f}%")
                        print(
                            f"   High Confidence (≥95%): {high_conf_count} ({high_conf_count/len(confidences)*100:.1f}%)"
                        )
                        ...
                        if doc_stats:
                            ...
                            print(
                                f"   Top Performer: {sorted_docs[0][0]} ({sorted_docs[0][1]['success']}/{sorted_docs[0][1]['total']})"
                            )
...
def generate_progress_summary(results_data):
    ...
    try:
        ...
        confidence_ranges = {"95-100%": 0, "90-95%": 0, "85-90%": 0, "80-85%": 0, "70-80%": 0, "<70%": 0}
