...
class ComprehensiveRAGTester:
            ...
            for i, template in enumerate(template_set):
                if i < 22:  # Limit to 22 questions per document
                    ...
                    doc_questions.append({
                        'question': question,
                        'expected_keywords': keywords + [doc_name.split()[0], doc_name.split()[1]] if len(doc_name.split()) > 1 else keywords,
        ...
        for i, q_data in enumerate(questions, 1):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    keywords_matched = sum(1 for kw in expected_keywords 
                                         if kw.lower() in answer.lower())
                    
                    # Determine success criteria
                    success = (
                        confidence and confidence >= 0.95 and  # 95%+ confidence
                        keywords_matched >= len(expected_keywords) // 2 and  # At least half keywords
                        len(answer) > 50  # Substantial answer
                    )
                    
                    reasoning = self.generate_reasoning(confidence, keywords_matched, 
                                                     len(expected_keywords), sources_count, len(answer))
    ...
    def generate_reasoning(self, confidence: Optional[float], keywords_matched: int, 
                          total_keywords: int, sources_count: int, answer_length: int) -> str:
    ...
    def display_result(self, result: TestResult):
        ...
        print(f"   {status} | Confidence: {conf_display} | Sources: {result.sources_found} | Keywords: {result.keywords_matched}")
    ...
    def display_final_summary(self, summary: Dict[str, Any]):
        ...
        print(f"   High Confidence (≥95%): {summary['high_confidence_tests']} ({summary['high_confidence_rate']*100:.1f}%)")
        ...
        for doc, analysis in summary['document_analysis'].items():
            ...
            print(f"   {status} {doc}")
            print(f"      Success: {analysis['passed_questions']}/{analysis['total_questions']} ({analysis['success_rate']*100:.1f}%)")
