...
test.describe('Performance smoke', () => {
  test('landing page loads (probe)', async ({ page, browserName }) => {
    const t0 = Date.now();
    try {
      ...
      console.warn(`[playwright] landing load failed: ${error}`);
      throw error;
    }
  });
  ...
  test('chat latency', async ({ request }) => {
    ...
    const response = await request.post(`${BASE_URL}/api/chat`, {
      headers: {
        ...
        Authorization: process.env.PLAYWRIGHT_BEARER_TOKEN ? `Bearer ${process.env.PLAYWRIGHT_BEARER_TOKEN}` : '',
      },
