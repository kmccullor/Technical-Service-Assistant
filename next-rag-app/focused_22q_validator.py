...
class FocusedRAGValidator:
    ...
    def display_summary(self, report: Dict[str, Any]):
        ...
        print(f"   High Confidence (≥95%): {stats['high_confidence_count']}/{len([r for r in self.results if r.confidence])} ({stats['high_confidence_rate']*100:.1f}%)")
        target_status = "✅ TARGET MET" if target['target_met'] else "⚠️ APPROACHING TARGET" if stats['high_confidence_rate'] >= 0.60 else "❌ TARGET NOT MET"
        ...
        for doc, perf in report['document_performance'].items():
            ...
            print(f"   📄 {doc[:50]}...")
            print(f"      Success: {perf['successful']}/22 ({success_rate:.1f}%) | Avg Conf: {avg_conf:.1f}% | ≥95%: {high_conf}")
