...
from utils.rbac_models import (
    ...
    PermissionLevel, UserStatus, PaginatedResponse, ForcePasswordChangeRequest
)
...
@rbac_router.get("/me", response_model=UserResponse, tags=["Authentication"])
async def me(
    current_user: User = Depends(get_current_active_user),
    auth_manager: AuthManager = Depends(get_auth_manager)
):
    """Return current authenticated user's profile including password change requirement."""
    # Attempt to fetch role name for completeness
    role_name = None
    try:
        conn = await auth_manager.get_db_connection(); cursor = conn.cursor()
        try:
            cursor.execute("SELECT name FROM roles WHERE id=%s", (current_user.role_id,))
            row = cursor.fetchone()
            if row:
                role_name = row['name'] if isinstance(row, dict) else row[0]
        finally:
            cursor.close(); conn.close()
    except Exception:
        pass

    # Provide defaults if timestamps missing
    ...
    try:
        ...
        users_query = f"""
            SELECT u.id, u.email, u.first_name, u.last_name, u.role_id,
                   ...
                   u.password_change_required, r.name as role_name
            FROM users u
...
async def get_user(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve user") from e
    ...
    # Defensive handling for optional fields that UserResponse requires
    if user.id is None:
        raise HTTPException(status_code=500, detail="Corrupt user record (missing id)")
...
@rbac_router.post("/force-change-password", response_model=APIResponse, tags=["Authentication"])
async def force_change_password(
    password_data: ForcePasswordChangeRequest,
    current_user: User = Depends(get_current_active_user),
    auth_manager: AuthManager = Depends(get_auth_manager)
):
    """Force required initial password change (old password not required).

    Accepts ChangePasswordRequest for schema reuse but ignores current_password.
    """
    try:
        ...
        success = await auth_manager.force_password_change(user_id, password_data.new_password)
        if success:
        ...
        logger.error(f"Forced password change failed for user {getattr(current_user, 'id', 'unknown')}: {e}")
