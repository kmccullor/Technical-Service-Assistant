...
class AcronymExtractor:
    ...
    def __init__(self):
        ...
        self.parentheses_pattern = re.compile(
            r'\b([A-Z][a-zA-Z\s&-]{3,50}?)\s*\(([A-Z]{2,8})\)', 
            re.MULTILINE
        )
        ...
        self.colon_pattern = re.compile(
            r'\b([A-Z]{2,8})\s*:\s*([A-Z][a-zA-Z\s,.-]{5,80})(?:[.!?]|\s|$)',
            re.MULTILINE
        )
        ...
        self.filter_words = {
            ...
            'DID', 'YES', 'HIS', 'HAS', 'LET', 'PUT', 'TOO', 'OLD', 'ANY', 'APP',
            'SUN', 'SET', 'RUN', 'HOT', 'CUT', 'LET', 'FUN', 'END', 'WHY', 'OFF'
        }
        
        # Technical domain indicators (increase confidence for acronyms)
        self.technical_indicators = {
            'protocol', 'interface', 'system', 'network', 'service', 'application',
            'database', 'server', 'client', 'API', 'SDK', 'framework', 'library',
            'standard', 'specification', 'technology', 'platform', 'architecture'
        }
    ...
    def _extract_contextual_acronyms(self, text: str) -> Dict[str, str]:
        ...
        context_patterns = [
            re.compile(r'\b(?:the|a|an)\s+([A-Z]{2,6})\s+(system|protocol|interface|service|application|network|database|server|platform)\b', re.IGNORECASE),
