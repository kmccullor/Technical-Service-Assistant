...
from utils.rbac_models import PermissionCheckResponse, PermissionLevel, User
...
class PermissionCache:
    ...
    def set(self, user_id: int, permissions: List[str]):
        ...
        self.cache[user_id] = {"permissions": permissions, "timestamp": datetime.utcnow()}
...
class RateLimiter:
    ...
    def get_remaining_requests(self, identifier: str) -> int:
        ...
        now = datetime.utcnow()
        window_start = now - timedelta(seconds=self.window_seconds)
        current_requests = [req_time for req_time in self.requests[identifier] if req_time > window_start]
...
class RBACManager:
    ...
    async def check_permission(
        self, user_id: int, permission: str, resource: Optional[str] = None, context: Optional[Dict[str, Any]] = None
    ...
    async def log_failed_permission_check(
        ...
        self.failed_permission_checks[user_id] = [
            attempt for attempt in self.failed_permission_checks[user_id] if attempt > window_start
        ]
    ...
    async def check_multiple_permissions(
        self, user_id: int, permissions: List[str], require_all: bool = True
    ) -> Dict[str, bool]:
...
class RBACMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if self.is_public_endpoint(path):
                ...
                await self.log_request(request, response, start_time, None, client_ip, user_agent)
                return response
            ...
            if path in self.rate_limited_endpoints:
                ...
                if not rate_limiter.is_allowed(client_ip):
                    await self.rbac_manager.log_security_event(
                        ...
                        details={"endpoint": path, "method": method, "user_agent": user_agent},
                    )
                    ...
                    return JSONResponse(
                        ...
                        content={
                            ...
                            "details": {"retry_after": self.rate_limited_endpoints[path].window_seconds},
                        },
                    )
            ...
            if auth_header and auth_header.lower().startswith("bearer "):
                ...
                try:
                    ...
                    auth_manager = (
                        await manager_candidate if inspect.iscoroutine(manager_candidate) else manager_candidate
                    )
            ...
            await self.log_request(request, error_response, start_time, str(e), client_ip, user_agent)
    ...
    async def log_request(
        ...
        logger.info(f"{request.method} {request.url.path} - " f"{response.status_code} - {duration:.3f}s - {client_ip}")
            ...
            if permissions:
                ...
                if require_all:
                    if not all(permission_results.values()):
                        missing_permissions = [perm for perm, has_perm in permission_results.items() if not has_perm]
                        raise HTTPException(
                            status_code=status.HTTP_403_FORBIDDEN,
...
def require_role(*roles: str):
    ...
    async def role_dependency(
        current_user: User = Depends(get_current_user), rbac_manager: RBACManager = Depends(get_rbac_manager)
    ) -> User:
        ...
        if not has_required_role:
            await rbac_manager.log_security_event(
                ...
                details={"required_roles": list(roles), "user_roles": user_role_names},
            )
            raise HTTPException(
