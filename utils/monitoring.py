...
try:
    ...
    OPERATION_COUNTER = Counter(
        "technical_service_operations_total", "Total number of operations performed", ["operation_type", "status"]
    )

    OPERATION_DURATION = Histogram(
        "technical_service_operation_duration_seconds", "Time spent on operations", ["operation_type"]
    )

    ACTIVE_OPERATIONS = Gauge(
        "technical_service_active_operations", "Number of currently active operations", ["operation_type"]
    )
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            try:
                ...
                if PROMETHEUS_AVAILABLE:
                    OPERATION_COUNTER.labels(operation_type=operation_type, status="error").inc()
...
def performance_context(operation_name: str):
    ...
    try:
        ...
        if PROMETHEUS_AVAILABLE:
            OPERATION_COUNTER.labels(operation_type=operation_name, status="success").inc()
        ...
        if PROMETHEUS_AVAILABLE:
            OPERATION_COUNTER.labels(operation_type=operation_name, status="error").inc()
