...
class EnhancedHybridSearch:
    ...
    def analyze_query(self, query: str, base_threshold: float = 0.5) -> QueryAnalysis:
        ...
        temporal_indicators = [indicator for indicator in self.temporal_indicators if indicator in query_lower]
        ...
        search_strategy = self._determine_search_strategy(complexity, temporal_indicators, domain_keywords)
    ...
    def _classify_question_type(self, query: str) -> str:
        ...
        if any(
            term in query_lower
            for term in ["install", "config", "setup", "active directory", "server", "network", "deployment"]
        ):
            return "technical"

        # Programming/code keywords
        if any(
            term in query_lower for term in ["code", "function", "algorithm", "programming", "python", "javascript"]
        ):
            return "code"

        # Math/calculation keywords
        if any(term in query_lower for term in ["calculate", "solve", "derivative", "integral", "equation", "math"]):
            return "math"

        # Creative keywords
        if any(term in query_lower for term in ["write", "create", "design", "story", "poem"]):
            return "creative"

        # Chat/conversational
        if any(term in query_lower for term in ["hello", "hi", "how are you", "good morning"]):
            return "chat"
    ...
    def _calculate_adaptive_threshold(
        self, complexity: QueryComplexity, question_type: str, temporal_indicators: List[str], base_threshold: float
    ) -> float:
    ...
    def _determine_search_strategy(
        self, complexity: QueryComplexity, temporal_indicators: List[str], domain_keywords: List[str]
    ) -> SearchStrategy:
    ...
    def _calculate_rerank_priority(self, complexity: QueryComplexity, question_type: str) -> float:
        ...
        type_bonus = {"code": 0.3, "technical": 0.2, "math": 0.25, "factual": 0.1, "creative": 0.0, "chat": -0.1}
        base_priority += type_bonus.get(question_type, 0.0)

        return max(0.0, min(1.0, base_priority))

    def calculate_enhanced_confidence(
        self, query: str, context_chunks: List[str], response: str, query_analysis: QueryAnalysis
    ) -> float:
        ...
        factors = self.type_confidence_factors.get(question_type, self.type_confidence_factors["factual"])
        ...
        quality_indicators = {
            ...
            "authoritative": any(
                word in response_lower for word in ["according to", "research shows", "studies indicate"]
            ),
        }
        ...
        partial_matches = len(
            [term for term in query_terms if any(term in chunk_word for chunk_word in context_text.split())]
        )
        ...
        if query_analysis.domain_keywords:
            ...
            domain_terms_in_context = sum(
                1 for keyword in query_analysis.domain_keywords if keyword.split(":")[1] in context_text
            )
