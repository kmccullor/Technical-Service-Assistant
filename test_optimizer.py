...
class TestSuiteAnalyzer:
    ...
    def analyze_test_class(self, node: ast.ClassDef) -> Dict:
        ...
        return {
            ...
            "has_setup": any(
                method.name in ["setUp", "setup_method"] for method in node.body if isinstance(method, ast.FunctionDef)
            ),
            "has_teardown": any(
                method.name in ["tearDown", "teardown_method"]
        ...
        for file_path, file_analysis in self.test_metrics.get("file_analysis", {}).items():
            for test_func in file_analysis.get("test_functions", []):
                ...
                test_signatures[signature].append(
                    {"file": file_path, "test": test_func["name"], "complexity": test_func["complexity"]}
                )

        # Find duplicates
        for signature, tests in test_signatures.items():
            if len(tests) > 1:
                duplicates.append({"signature": signature, "tests": tests, "duplicate_count": len(tests)})
...
class FlakyTestDetector:
    ...
    def parse_test_execution_results(self, stdout: str, run_id: int) -> Dict:
        ...
        run_data = {"run_id": run_id, "timestamp": datetime.now().isoformat(), "tests": {}}
        ...
        for test_name, executions in execution_data.items():
            ...
            if 0 < failure_rate < 1.0 and failed_runs >= 1:
                ...
                flaky_test = {
                    ...
                    "last_failure": max(
                        [exec["timestamp"] for exec in executions if exec["status"] == "failed"], default=None
                    ),
                }
...
class TestOptimizer:
    ...
    def generate_optimization_recommendations(self, suite_analysis: Dict) -> List[Dict]:
        ...
        for file_path, file_analysis in suite_analysis["file_analysis"].items():
            ...
            # Complex tests
            high_complexity_tests = [test for test in file_analysis.get("test_functions", []) if test["complexity"] > 5]
    ...
    def identify_performance_improvements(self, suite_analysis: Dict) -> List[Dict]:
        ...
        for file_path, file_analysis in suite_analysis["file_analysis"].items():
            slow_tests = [test for test in file_analysis.get("test_functions", []) if test["estimated_runtime"] > 1.0]
        ...
        for file_path, file_analysis in suite_analysis["file_analysis"].items():
            for test in file_analysis.get("test_functions", []):
                if test["complexity"] > 3 and not test["mock_usage"]:
                    ...
                    fast_tests.append(
                        {"file": file_path, "test": test["name"], "estimated_runtime": test["estimated_runtime"]}
                    )
    ...
    def apply_optimizations(self, optimization_results: Dict) -> Dict:
        ...
        applied_optimizations = {"automated_fixes": 0, "manual_recommendations": 0, "performance_improvements": 0}
    ...
    if args.analyze:
        ...
        if flaky_tests:
            ...
            for flaky in flaky_tests[:10]:  # Show top 10
                print(
                    f"   - {flaky['test_name']}: {flaky['failure_rate']:.1%} failure rate "
