...
class DocumentAccuracyTester:
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            if test_case['expected_type']:
                ...
                if result and result['document_type'] == test_case['expected_type']:
                    ...
                    print(f"❌ FAIL: Expected {test_case['expected_type']}, got {result['document_type'] if result else 'None'}")
            else:
    ...
    def test_query_expansion_accuracy(self):
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            if expansion_rate >= 0.5:  # At least 50% of expected terms found
                print(f"✅ PASS: Found {len(found_expansions)}/{len(test_case['expected_expansions'])} expected expansions")
                print(f"   Expanded: '{expanded_query}'")
                successful_expansions += 1
            else:
                print(f"❌ FAIL: Only found {len(found_expansions)}/{len(test_case['expected_expansions'])} expected expansions")
    ...
    def print_final_report(self):
        ...
        print(f"\n🚀 IMPROVEMENT IMPACT:")
        print(f"   Security Documents: 42.9% → {self.test_results['security_classification']['accuracy']:.1f}% (+{self.test_results['security_classification']['accuracy'] - 42.9:.1f}pp)")
        print(f"   Query Expansion: 55.0% → {self.test_results['query_expansion']['accuracy']:.1f}% (+{self.test_results['query_expansion']['accuracy'] - 55.0:.1f}pp)")
        print(f"   Metadata Extraction: 22.6% → {self.test_results['metadata_extraction']['accuracy']:.1f}% (+{self.test_results['metadata_extraction']['accuracy'] - 22.6:.1f}pp)")
