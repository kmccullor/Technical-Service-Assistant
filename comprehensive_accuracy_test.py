...
def test_semantic_search_accuracy():
    ...
    for i, test in enumerate(test_queries, 1):
        ...
        try:
            ...
            overall_accuracy = (
                type_accuracy * 0.4 +          # 40% weight on document type accuracy
                product_accuracy * 0.3 +       # 30% weight on product accuracy
                (1 - avg_distance) * 0.2 +     # 20% weight on semantic similarity (lower distance = higher accuracy)
                keyword_accuracy * 0.1         # 10% weight on keyword matching
            )
            
            accuracy_scores.append(overall_accuracy)
            
            print(f"\n📈 Accuracy Metrics:")
            print(f"   Document Type: {type_accuracy:.1%} ({len(set(test['expected_types']) & doc_types_found)}/{len(test['expected_types'])} types matched)")
            print(f"   Product Match: {product_accuracy:.1%} ({len(set(test['expected_products']) & products_found)}/{len(test['expected_products'])} products matched)")
