    ...
    def import_database_schema(
        ...
        try:
            ...
            import_stats = self._import_schema_data(
                rni_version_id=rni_version_id, schema_data=schema_data, conn_config=conn_config, created_by=created_by
            )
    ...
    def _import_schema_data(
        ...
        try:
            with conn.cursor() as cursor:
                ...
                database_instance_id = self._create_database_instance(cursor, rni_version_id, conn_config, schema_data)
                stats["database_instances"] = 1

                # Process each schema
                for schema_name in schema_data["schemas"]:
                    schema_id = self._create_database_schema(cursor, database_instance_id, schema_name)
                    stats["schemas"] += 1

                    # Process tables in this schema
                    schema_tables = {k: v for k, v in schema_data["tables"].items() if k.startswith(f"{schema_name}.")}

                    for table_key, table_data in schema_tables.items():
                        table_info = table_data["table_info"]

                        # Create table record
                        table_id = self._create_database_table(cursor, schema_id, table_info)
                        stats["tables"] += 1

                        # Create columns
                        for column_info in table_data["columns"]:
                            self._create_database_column(cursor, table_id, column_info)
                            stats["columns"] += 1

                        # Create constraints
                        for constraint_info in table_data["constraints"]:
                            self._create_table_constraint(cursor, table_id, constraint_info)
                            stats["constraints"] += 1

                        # Create indexes
                        for index_info in table_data["indexes"]:
                            self._create_table_index(cursor, table_id, index_info)
                            stats["indexes"] += 1
                ...
                self._create_change_log_entry(
                    ...
                    "CREATE",
                    "SCHEMA_IMPORT",
                    f"Complete schema import for {conn_config.database}",
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                if not db_instance:
                    ...
                    cursor.execute(
                        ...
                        (rni_version_id, database_name, database_type, f"Imported from CSV upload", True),
                    )
                    db_instance = cursor.fetchone()
                ...
                # Create change log entry
                change_description = (
                    f"CSV schema import: {total_schemas} schemas, {total_tables} tables, {total_columns} columns"
                )
                self._create_change_log_entry(
                    cursor, rni_version_id, database_instance_id, "CREATE", "SCHEMA", change_description, created_by
                )
