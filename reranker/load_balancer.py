...
class OllamaLoadBalancer:
    ...
    def __init__(self, instance_urls: Optional[List[str]] = None):
        ...
        self.metrics: Dict[str, InstanceMetrics] = {
            url: InstanceMetrics(instance_url=url) for url in instance_urls
        }
    ...
    async def get_next_instance(self, request_type: RequestType = RequestType.INFERENCE) -> str:
        ...
        healthy_instances = [
            ...
            if self.metrics[url].is_healthy
        ]
        ...
        if healthy_instances:
            ...
            logger.debug(f"Selected {selected_url} for {request_type.value} (score: {healthy_instances[0][1].get_load_score():.2f})")
            return selected_url
    ...
    async def get_healthy_instances(self, request_type: RequestType = RequestType.INFERENCE) -> List[str]:
        ...
        healthy = [
            url for url in self.instance_urls 
            if self.metrics[url].is_healthy
        ]
        return healthy if healthy else self.instance_urls
    
    def record_request(self, instance_url: str, response_time: float, success: bool, error: Optional[str] = None) -> None:
