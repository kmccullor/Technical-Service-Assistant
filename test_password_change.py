...
def test_login_with_password_change_required():
    ...
    # Test Kevin's login
    login_data = {"email": "kevin.mccullor@xylem.com", "password": "password123"}
...
def test_force_password_change(access_token):
    ...
    password_data = {"new_password": "NewSecurePass123!", "confirm_password": "NewSecurePass123!"}
    import json

    response = requests.post(f"{BASE_URL}/force-change-password", data=json.dumps(password_data), headers=headers)
...
def test_login_after_password_change():
    ...
    login_data = {"email": "kevin.mccullor@xylem.com", "password": "NewSecurePass123!"}
