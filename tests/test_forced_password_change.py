...
def test_force_password_change_endpoint(client, flagged_user):
    ...
    force_resp = client.post(
        "/api/auth/force-change-password", json={"new_password": "NewSecurePass123!"}, headers=headers
    )
...
def test_login_includes_password_change_flag(client, flagged_user):
    ...
    resp = client.post("/api/auth/login", json={"email": flagged_user.email, "password": "testpass"})
