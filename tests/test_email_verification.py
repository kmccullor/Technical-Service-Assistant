...
async def test_verify_email_success(client):
    ...
    with patch("reranker.rbac_endpoints.get_auth_manager", return_value=mock_auth), patch(
        "rbac_endpoints.get_auth_manager", return_value=mock_auth
    ):  # dual patch for path variance
