...
def enhanced_query_expansion(query: str) -> str:
    ...
    for context_term, patterns in CONTEXT_PATTERNS.items():
        if context_term in query_lower:
            ...
            is_problem_context = any(problem_word in query_lower 
                                   for problem_word in ['problem', 'issue', 'error', 'fail', 'trouble'])
...
def test_phase2_improvements():
    ...
    for i, query in enumerate(test_queries, 1):
        ...
        has_troubleshoot_terms = any(term in expanded_lower 
                                   for term in ['troubleshoot', 'debug', 'error', 'issue', 'problem', 'fix'])
